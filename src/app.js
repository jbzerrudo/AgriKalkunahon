...
const T = {
  ...
  cards: {
    ...
    disease: { en: 'Will the leaves be wet in the morning?', fil: 'Mababasâ ba ang dahon sa umaga?' },
  ...
  ui: {
    ...
    daysSince: { en: 'Days since the soil was last soaked (rain or irrigation)', fil: 'Ilang araw mula nang huling mabasâ nang husto ang lupa' },
  ...
  verdicts: {
    ...
    moisture_out_of_range: { en: 'Check the moisture reading. It must be between 1 and 99%.', fil: 'Suriin ang basâ ng palay. Dapat nasa 1 hanggang 99%.' },
    ...
    dew_very_likely_near_saturation: { en: 'Wet leaves at dawn very likely. The air only needs to cool {dep} °C, which happens on almost any night.', fil: 'Halos tiyak na basâ ang dahon sa madaling-araw. Kailangan lang lumamig ng {dep} °C ang hangin, na nangyayari halos tuwing gabi.' },
...
const CODES = {
  ...
  awd_start_window_unknown: 'If your crop is less than 21 days old, ignore the answer above and keep 2 to 3 cm of water instead. That is the depth for a newly transplanted crop, not a shallower version of the re-flood depth: IRRI sets it by the height of the plant, "around 3 cm initially, and gradually increase to 5-10 cm (with increasing plant height)". AWD itself should not begin until 21 to 30 days after transplanting or sowing (PhilRice; DA AO 25-09: 20 to 30 days). Without a transplanting date this card cannot check that for you, so give it one and it will.',
...
/* onchange(reason) is told why the location changed: 'gps' for the locate button, 'latlon' for a typed
   coordinate, 'other' for elevation or site. climateExtra, if given, adds a card's own words to the
   climate type line (the rice card adds what to set Season to). */
function locationBlock(onchange, climateExtra) {
  const L = store.loc;
  ...
  const gps = el('button', { type: 'button', class: 'btn small', onclick: () => {
    if (!navigator.geolocation) return gpsFailed({ en: 'This browser cannot read your location. Type the numbers in below.', fil: 'Hindi kayang basáhin ng browser na ito ang lokasyon ninyo. I-type ang mga numero sa ibaba.' });
    navigator.geolocation.getCurrentPosition(
      p => { const n = v => (typeof v === 'number' && isFinite(v)) ? v : null;   // isFinite(null) is true, so test the type too
             ...
             L.acc = n(p.coords.accuracy); L.altAcc = n(p.coords.altitudeAccuracy); L.altFromGps = n(p.coords.altitude) != null; sync('gps'); },
  ...
  function sync(reason) { L.lat = +lat.input.value; L.lon = +lon.input.value; L.elev = +elev.input.value; L.site = site.input.value; L.set = true; save(); redraw(); if (onchange) onchange(reason); }
  [lat, lon, elev].forEach(x => x.input.addEventListener('input', () => { L.acc = null; L.altAcc = null; L.altFromGps = false; }));
  /* The PAGASA climate type, right under what the locate button fills in, once the location is the
     farmer's own. Outside the Philippines the warning below already says so, and nothing is added. */
  ...
  const warn = el('div');
  [lat, lon].forEach(x => x.input.addEventListener('change', () => sync('latlon')));
  [elev, site].forEach(x => x.input.addEventListener('change', () => sync('other')));
  wrap.append(gps, note, hint, lat.row, lon.row, elev.row, ctLine, site.row, warn);
  redraw();
  wrap.redraw = redraw;
  return wrap;
}
...
function gpsUncertainty() {
  ...
  if (typeof L.acc !== 'number' || !isFinite(L.acc)) return {
    ...
    fil: 'Itinipa ang mga koordinadang ito, kaya walang alam ang app na fix mula sa satellite. Binabása ito hanggang sandaan ng digri lamang, mga ' + grid0 + ' m sa lupa, at hindi hihigit doon ang kanyang tinutukoy.' + altF
  };
...
CARDS.water = function (root) {
  ...
  const tmax = numInput('tmax', T.ui.tmax, prev.tmax, 0.1, { signed: true }), tmin = numInput('tmin', T.ui.tmin, prev.tmin, 0.1, { signed: true });
  ...
  for (let k = 1; k <= FC_DAYS; k++) {
    const fx = numInput('wFx' + k, { en: 'High (°C)', fil: 'Pinakamataas na temperatura (°C)' }, prev['fx' + k], 0.1, { signed: true });
    const fn = numInput('wFn' + k, { en: 'Low (°C)', fil: 'Pinakamababang temperatura (°C)' }, prev['fn' + k], 0.1, { signed: true });
  ...
  function run() {
    ...
    for (let k = 1; k <= FC_DAYS; k++) {
      ...
      if (hx < hn) { out.appendChild(result({ level: 'info', verdict: { en: 'Forecast day ' + k + ': the high must be above the low.', fil: 'Araw ' + k + ' ng forecast: dapat mas mataas ang pinakamataas na temperatura kaysa sa pinakamababang temperatura.' } })); return; }
    ...
    if (!fdays.length) { out.appendChild(result({ level: 'info', verdict: { en: 'Enter at least the high and low for the day after the date above to use the forecast.', fil: 'Ilagay man lang ang pinakamataas at pinakamababang temperatura sa araw pagkatapos ng petsa sa itaas para magamit ang forecast.' } })); return; }
...
CARDS.rice = function (root) {
  ...
  const levelDate = dateInput('leveldate', { en: 'Date you took that reading.', fil: 'Petsa ng pagbása mo ng iyong inilagay na datos.' }, prev.levelDate, { prefilled: true });
  const levelPrevDate = dateInput('levelprevdate', { en: 'Date of the earlier reading. The further back it is, the tighter the date below. Wait until the water has fallen at least about 3 cm before reading again.', fil: 'Petsa ng naunang pagbása. Mas malayo ito, mas tiyak ang petsa sa ibaba. Hintayin munang bumaba ang tubig nang hindi bababa sa mga 3 cm bago magbása muli.' }, prev.levelPrevDate, { optional: true });
  ...
  const tmax = numInput('rtmax', { en: 'Afternoon high on the day of that reading, deg C. With the morning low, the card works out crop water use and can split your loss into its parts.', fil: 'Pinakamainit sa araw ng pagbásang iyon, deg C. Kasama ang pinakamalamig sa umaga, matatantiya ng card ang gamit na tubig ng pananim, at malalaman kung magkano ang tagas at tagimtim.' }, prev.tmax, 0.1, { optional: true, signed: true });
  const tmin = numInput('rtmin', { en: 'Morning low on that same day, deg C.', fil: 'Pinakamalamig sa umaga ng araw na iyon, deg C.' }, prev.tmin, 0.1, { optional: true, signed: true });
  ...
  /* The PAGASA climate type at the saved location, looked up on the IAAS shapefile [PAGASA_IAAS_CTYPE],
     sets the season. A new location, from the locate button or typed in, sets it at once wherever the map
     can say; the farmer can still change it, and that choice holds until the location changes again.
     Where the map cannot say, the season goes back to the farmer's own choice, or the dry-season default. */
  const ctNote = el('div', { class: 'ctnote' });
  const farmerSeasonOf = p => (p.season != null && (p.seasonFrom === 'farmer' || p.seasonFrom == null)) ? p.season : null;
  let farmerSeason = farmerSeasonOf(prev), seasonAuto = false, locBlock = null;
  const climateHere = () => {
    const L = store.loc;
  ...
  const syncClimate = fromLocation => {
    ctNote.hidden = season.row.hidden;
    const c = climateHere(), sug = c.status === 'ok' && c.cs ? c.cs.season : null;
    if (fromLocation || seasonAuto || farmerSeason == null) {
      if (sug) { season.input.value = sug; seasonAuto = true; }
      else { season.input.value = farmerSeason || 'dry'; seasonAuto = false; }
    }
    ctNote.innerHTML = '';
    ctNote.appendChild(el('p', { class: 'lochint' }, bi(seasonNote(c, season.input.value, seasonAuto))));
    if (locBlock) locBlock.redraw();
  };
  season.input.addEventListener('change', () => { farmerSeason = season.input.value; seasonAuto = false; syncClimate(false); });
  levelDate.input.addEventListener('change', () => syncClimate(false));
  locBlock = locationBlock(reason => syncClimate(reason === 'gps' || reason === 'latlon'), () => {
    if (season.row.hidden) return null;
    const c = climateHere();
    return c.status === 'ok' ? riceSeasonAdvice(c, season.input.value, seasonAuto) : null;
  });
  ...
  const clear = el('button', { type: 'button', class: 'btn' }, bi({ en: 'Forget this paddy\'s past readings', fil: 'Burahin ang mga naunang pagbása sa palayang ito' }));
  clear.addEventListener('click', () => {
    if (!confirm(t({ en: 'Forget every past reading from this paddy? The card will go back to using only the two readings you type in.', fil: 'Burahin lahat ng naunang pagbása sa palayang ito? Babalik ang card sa paggamit lamang ng dalawang pagbásang ita-type mo.' }).en)) return;
  ...
  const syncForget = () => {
    ...
    if (!n && !rf) {
      memo.append(bi({ en: 'This card remembers each pair of readings you give it for this paddy, on this phone only. From them it works out your field\'s usual water loss and your usual gap between irrigations.',
                       fil: 'Naitatala ng card na ito ang bawat pares ng pagbásang ibibigay mo para sa palayang ito, sa smartphone na ito lamang (hindi lumalabas ang impormasyong naisalba rito). Mula rito, matatantiya ang karaniwang pagbaba ng tubig sa bukid mo at ang karaniwang agwat ng pagpapatubig.' }));
      return;
    }
    memo.append(bi({ en: 'Remembering ' + n + (n === 1 ? ' reading' : ' readings') + ' and ' + rf + (rf === 1 ? ' re-flood' : ' re-floods') + ' from paddy' + (named || ' (unnamed)') + ', the latest one included, on this phone only.',
                     fil: 'Naitala: ' + n + ' pagbása at ' + rf + ' pagpapatubig mula sa palayang' + (named || ' walang pangalan') + ', kasama ang pinakahuli sa smartphone na ito (hindi lumalabas ang impormasyong naisalba rito).' }), ' ', clear);
  };
  ...
  const loadPlot = () => {
    ...
    farmerSeason = farmerSeasonOf(prev); seasonAuto = false;
  ...
  function run() {
    ...
    const inp = { plot: plot.value, method: method.input.value, instrument: instrument.input.value,
                  ...
                  season: valOf(season, 'dry'), seasonFrom: season.row.hidden ? 'default' : (seasonAuto ? 'auto' : (farmerSeason != null ? 'farmer' : 'default')), est: est.input.value, flower: flower.input.value, harvest: harvest.input.value,
    ...
    if (r.gainCm != null) lines.push([bi({ en: 'Net gain since your earlier reading', fil: 'Naidagdag mula sa naunang pagbása' }), fmt(r.gainCm, 1) + ' cm: the water rose instead of falling']);
    ...
    if (ctShown) lines.push([bi({ en: 'PAGASA climate type here', fil: 'Uri ng klima rito ayon sa PAGASA' }), bi(climateLine(cHere, inp.season, inp.seasonFrom))]);
...
const CT_DESC = {
  1: { en: 'Two pronounced seasons: dry from November to April and wet the rest of the year, with the heaviest rain from June to September.', fil: 'Dalawang malinaw na panahon: tag-araw mula Nobyembre hanggang Abril at tag-ulan sa natitirang bahagi ng taon, at pinakamalakas ang ulan mula Hunyo hanggang Setyembre.' },
  2: { en: 'No dry season, with the heaviest rain from December to February; not a single month is dry.', fil: 'Walang tag-araw, at pinakamalakas ang ulan mula Disyembre hanggang Pebrero; walang kahit isang tuyong buwan.' },
  3: { en: 'No very pronounced period of heaviest rain, and a short dry season of one to three months, somewhere in December to February or March to May.', fil: 'Walang tiyak na panahon ng pinakamalakas na ulan, at maikli ang tag-araw, isa hanggang tatlong buwan, sa loob ng Disyembre hanggang Pebrero o Marso hanggang Mayo.' },
  4: { en: 'Rain more or less evenly spread through the year, and no dry season.', fil: 'Halos pantay ang ulan sa buong taon, at walang tag-araw.' }
};
...
function climateTypeText(ct) {
  ...
  if (ct.nearTypes.length) {
    en += ' ' + ctList(ct.nearTypes, ' and ') + (ct.nearTypes.length > 1 ? ' lie' : ' lies') + ' within about 5 km of here.';
    fil += ' May ' + ctList(ct.nearTypes, ' at ') + ' din sa loob ng mga 5 km mula rito.';
  }
  if (ct.map2014 != null) {
    en += ' The printed Climate Map of the Philippines (August 2014) shows Type ' + R[ct.map2014] + ' around here.';
    fil += ' Type ' + R[ct.map2014] + ' naman ang nakalagay rito sa nakalimbag na Climate Map of the Philippines (Agosto 2014).';
  }
  return { en: en, fil: fil };
}
/* What the rice card adds to that line: what Season has been set to, or why the farmer must set it. */
function riceSeasonAdvice(c, chosen, auto) {
  const cs = c.cs; if (!cs) return null;
  const now = fieldNow().getMonth() + 1, S = CT_SEASON, sug = cs.season, ch = S[chosen] || S.dry;
  const mE = MONTH_NAMES.en[c.month - 1] + (c.month !== now ? ' (the month of your reading)' : '');
  const mF = MONTH_NAMES.fil[c.month - 1] + (c.month !== now ? ' (ang buwan ng pagbása ninyo)' : '');
  if (sug) {
    const why = {
      no_dry_season: { en: 'With no dry season,', fil: 'Dahil walang tag-araw,' },
      type_i_dry_months: { en: mE + ' is in its dry season, so', fil: 'Tag-araw ang ' + mF + ' sa uring ito, kaya' },
      type_i_wet_months: { en: mE + ' is in its wet season, so', fil: 'Tag-ulan ang ' + mF + ' sa uring ito, kaya' },
      type_iii_wet_months: { en: mE + ' is in its wet season, since its dry months fall between December and May, so', fil: 'Tag-ulan ang ' + mF + ', dahil nasa pagitan ng Disyembre at Mayo ang mga buwan ng tag-araw nito, kaya' }
    }[cs.code];
    const tail = chosen !== sug
      ? { en: ' it calls for "' + S[sug].en + '", but Season below is set to "' + ch.en + '", as you chose.', fil: ' "' + S[sug].fil + '" ang hinihingi nito, ngunit "' + ch.fil + '" ang nakatakdang Panahon sa ibaba, ayon sa pinili ninyo.' }
      : auto ? { en: ' the card has set Season below to "' + S[sug].en + '" for you.', fil: ' itinakda na ng card ang Panahon sa ibaba sa "' + S[sug].fil + '" para sa inyo.' }
      : { en: ' Season below is set to "' + S[sug].en + '".', fil: ' nakatakda ang Panahon sa ibaba sa "' + S[sug].fil + '".' };
    return { en: why.en + tail.en, fil: why.fil + tail.fil };
  }
  if (cs.code === 'type_iii_months_not_given') return { en: 'The map does not say which months from December to May are dry here, so set Season below yourself: "Dry season" if ' + mE + ' is dry where you are, "Wet season" if not.', fil: 'Hindi sinasabi ng mapa kung aling mga buwan mula Disyembre hanggang Mayo ang tuyo rito, kaya kayo ang magtakda ng Panahon sa ibaba: "Tag-araw" kung tuyo ang ' + mF + ' sa lugar ninyo, "Tag-ulan" kung hindi.' };
  if (cs.code === 'near_other_type') { const n = cs.nearDiffer; return { en: 'Because ' + ctList(n, ' and ') + ' nearby ' + (n.length > 1 ? 'call' : 'calls') + ' for a different season in ' + mE + ', set Season below yourself.', fil: 'Dahil iba ang panahong hinihingi ng kalapit na ' + ctList(n, ' at ') + ' sa ' + mF + ', kayo ang magtakda ng Panahon sa ibaba.' }; }
  if (cs.code === 'versions_differ') return { en: 'Because the printed map\'s Type ' + CT_ROMAN[c.ct.map2014] + ' calls for a different season in ' + mE + ', set Season below yourself.', fil: 'Dahil iba ang panahong hinihingi ng Type ' + CT_ROMAN[c.ct.map2014] + ' sa nakalimbag na mapa sa ' + mF + ', kayo ang magtakda ng Panahon sa ibaba.' };
  return null;
}
...
function seasonNote(c, chosen, auto) {
  ...
  if (chosen !== sug) return { en: 'The PAGASA climate type at your location, Type ' + R + ', calls for "' + S[sug].en + '" (see Location above); you have chosen "' + ch.en + '".', fil: '"' + S[sug].fil + '" ang hinihingi ng uri ng klima sa lokasyon ninyo ayon sa PAGASA, Type ' + R + ' (tingnan ang Lokasyon sa itaas); "' + ch.fil + '" ang pinili ninyo.' };
  return auto ? { en: 'Set for you from the PAGASA climate type at your location, Type ' + R + ' (see Location above). Change it if your field is different.', fil: 'Itinakda para sa inyo batay sa uri ng klima sa lokasyon ninyo ayon sa PAGASA, Type ' + R + ' (tingnan ang Lokasyon sa itaas). Palitan kung iba ang sa bukid ninyo.' }
              : { en: 'This matches the PAGASA climate type at your location, Type ' + R + ' (see Location above).', fil: 'Tugma ito sa uri ng klima sa lokasyon ninyo ayon sa PAGASA, Type ' + R + ' (tingnan ang Lokasyon sa itaas).' };
}
function climateLine(c, chosen, from) {
  ...
  let en = 'Type ' + CT_ROMAN[ct.type] + ' (IAAS shapefile, DOST-PAGASA 2014, read at ' + fmt(ct.gridLat, 2) + ' N, ' + fmt(ct.gridLon, 2) + ' E). ' + CT_DESC[ct.type].en;
  let fil = 'Type ' + CT_ROMAN[ct.type] + ' (IAAS shapefile, DOST-PAGASA 2014, sa ' + fmt(ct.gridLat, 2) + ' N, ' + fmt(ct.gridLon, 2) + ' E). ' + CT_DESC[ct.type].fil;
  ...
  if (cs.season) {
    ...
    en += ' It does not settle the season in ' + mE + (from === 'farmer' ? ', so the card used the one you chose, "' + ch.en + '".' : ', so the card used its starting setting, "' + ch.en + '"; set Season yourself if that is wrong for your field.');
    fil += ' Hindi nito natitiyak ang panahon sa ' + mF + (from === 'farmer' ? ', kaya ang pinili ninyo ang ginamit ng card, "' + ch.fil + '".' : ', kaya ang panimulang setting ng card ang ginamit, "' + ch.fil + '"; kayo ang magtakda ng Panahon kung mali ito para sa bukid ninyo.');
  }
  return { en: en, fil: fil };
}
...
CARDS.rain = function (root) {
  ...
  const tx = numInput('rTx', { en: 'Typical afternoon high over the period (°C)', fil: 'Karaniwang pinakamataas na temperatura sa hapon sa panahong ito (°C)' }, prev.tx, 0.1, { signed: true });
  const tn = numInput('rTn', { en: 'Typical morning low over the period (°C)', fil: 'Karaniwang pinakamababang temperatura sa umaga sa panahong ito (°C)' }, prev.tn, 0.1, { signed: true });
  ...
  function run() {
    ...
    let sources = [];
    if (comp) {
      if (inp.tx == null || inp.tn == null) return info({ en: 'Enter the typical afternoon high and morning low over the period, so the card can work out the crop\'s water use.', fil: 'Ilagay ang karaniwang pinakamataas na temperatura sa hapon at pinakamababang temperatura sa umaga sa panahong ito, para makalkula ng card ang gamit na tubig ng pananim.' });
...
CARDS.dry = function (root) {
  ...
  const form = el('form', { class: 'card-form', onsubmit: e => { e.preventDefault(); run(); } });
  const Tn = numInput('dT', { en: 'Air temperature at the drying area (°C)', fil: 'Temperatura sa bilaran (°C)' }, prev.T, 0.1, { signed: true });
  const RH = numInput('dRH', { en: 'Humidity (%)', fil: 'Halumigmig (%)' }, prev.RH, 1);
  const w = numInput('dW', { en: 'Wet palay weight (kg), or leave blank', fil: 'Bigat ng basâng palay (kg), o iwanang blangko' }, prev.w, 1, { optional: true });
...
CARDS.stress = function (root) {
  ...
  const d0x = numInput('x0x', { en: 'Today\'s high (°C)', fil: 'Pinakamainit ngayon (°C)' }, prev.d0x, 0.1, { signed: true }), d0n = numInput('x0n', { en: 'Today\'s low (°C)', fil: 'Pinakamalamig ngayon (°C)' }, prev.d0n, 0.1, { signed: true });
  ...
  const fcRows = [1, 2, 3].map(k => ({ x: numInput('xF' + k + 'x', { en: (k === 1 ? 'Tomorrow' : 'In ' + k + ' days') + ': high (°C)', fil: (k === 1 ? 'Bukas' : 'Sa loob ng ' + k + ' araw') + ': pinakamataas na temperatura (°C)' }, prev['f' + k + 'x'], 0.1, { signed: true }),
                                       n: numInput('xF' + k + 'n', { en: (k === 1 ? 'Tomorrow' : 'In ' + k + ' days') + ': low (°C)', fil: (k === 1 ? 'Bukas' : 'Sa loob ng ' + k + ' araw') + ': pinakamababang temperatura (°C)' }, prev['f' + k + 'n'], 0.1, { signed: true }) }));
  ...
  function run() {
    ...
    if (!fdays.length) { out.appendChild(result({ level: 'info', verdict: { en: 'Enter at least tomorrow\'s forecast high and low to use the forecast.', fil: 'Ilagay man lang ang forecast na pinakamataas at pinakamababang temperatura bukas para magamit ang forecast.' } })); return; }
...
CARDS.frost = function (root) {
  ...
  const form = el('form', { class: 'card-form', onsubmit: e => { e.preventDefault(); run(); } });
  form.appendChild(locationBlock());
  const Tn = numInput('fT', { en: 'Air temperature at the time you read it (°C)', fil: 'Temperatura sa oras ng pagbása (°C)' }, prev.T, 0.1, { signed: true });
  ...
  function run() {
    ...
    const recText = v => v <= 0
      ...
      : v <= REC.hiC
      ? { en: fmt(v, 1) + ' °C: at or below ' + REC.hiC + ' °C, the warmest air on any of the ' + REC.days + ' frost days recorded at ' + REC.site + ' in ' + REC.year + ', which came with air of ' + REC.loC + ' to ' + REC.hiC + ' °C (Basquial et al. 2021).', fil: fmt(v, 1) + ' °C: abot o mas mababa sa ' + REC.hiC + ' °C, ang pinakamataas na temperatura ng hangin sa alinman sa ' + REC.days + ' araw na may naitalang andap sa ' + REC.site + ' noong ' + REC.year + ', na may hanging ' + REC.loC + ' hanggang ' + REC.hiC + ' °C (Basquial et al. 2021).' }
    ...
    const dpNow = A.dewPointNow(inp.T, inp.RH);
    const frostPointLine = dpNow.error ? null : (dpNow.frostPoint != null
      ? fmt(dpNow.frostPoint, 1) + ' \u00b0C, the temperature a surface must fall to for this air to deposit ice on it directly (Romps 2021)'
      : 'none for this air: its dew point is above freezing, so it would saturate over water first and what forms would be dew, not frost');
    const showDewPoint = function () {
      ...
      out.appendChild(result({ level: 'info',
        ...
        compare: dp.compare.map(function (c) { return c.name + ': ' + fmt(c.dewPoint, 2) + ' \u00b0C'; })
          .concat(['Spread across all four: ' + fmt(dp.compareSpreadC, 3) + ' \u00b0C. For comparison, a hygrometer reading three points of humidity off moves the dew point by about half a degree on a humid night, rising past 1 \u00b0C in warm dry air, and a thermometer off by half a degree moves it by about half a degree throughout. The method is not where the error is.']),
        ...
        limits: ['This describes the air where and when you read it. The dew point changes through the night, and a hollow does not hold the same air as the ridge above it, so one reading is a snapshot.',
                 ...
                 'The instrument sets the accuracy here, not the arithmetic. A hygrometer reading three points of humidity off moves the dew point by about half a degree in the damp air of a frost night, and by more than 1 \u00b0C in warm dry air; a thermometer off by half a degree moves it by about half a degree whatever the conditions. Both are far larger than any difference between the published methods.',
    ...
    if (!A.frostReadingUsable(nowH, stToday.sunset, st.sunrise)) {
      ...
      show(out, result({ level: 'info',
        ...
        lines: [[bi({ en: 'Read again after', fil: 'Magbása uli pagkatapos ng' }), hhmm(opens) + ' (sunset ' + hhmm(stToday.sunset) + ')'],
                [bi({ en: 'Coldest hour tonight', fil: 'Pinakamalamig na oras ngayong gabi' }), 'about ' + hhmm(st.sunrise - 1) + ' to ' + hhmm(st.sunrise) + ' (sunrise ' + hhmm(st.sunrise) + ')']].concat(inp.fmin != null ? [[bi({ en: 'PAGASA forecast minimum tonight', fil: 'Forecast ng PAGASA na pinakamababang temperatura ngayong gabi' }), bi(recText(inp.fmin)), inp.fmin <= REC.hiC ? 'alert' : '']] : []),
    ...
    lines.push([bi({ en: 'You read this at', fil: 'Oras ng pagbása' }), hhmm(nowH) + ', ' + fmt(toMin, 1) + ' hours before the coldest hour'],
      ...
      [bi({ en: 'Time of year', fil: 'Panahon ng taon' }), bi(seasonText)], [bi({ en: 'Dew point now', fil: 'Dew point ngayon' }), fmt(r.dewPoint, 1) + ' °C (line used: 2 °C)'],
      [bi({ en: 'Frost point', fil: 'Frost point' }), frostPointLine || '–'], [bi({ en: 'Radiation-frost conditions present', fil: 'Mga kondisyon ng andap na naroroon' }), ['clear sky', 'calm or light wind', 'low dew point', 'at or below 10 °C'].filter((_, i) => [c.clear, c.calm, c.lowDewPoint, c.cold][i]).join(', ') || 'none']);
    ...
    if (inp.fmin != null) {
      ...
      lines.push([bi({ en: 'PAGASA forecast minimum tonight', fil: 'Forecast ng PAGASA na pinakamababang temperatura ngayong gabi' }), bi({ en: recText(inp.fmin).en + ' ' + airTxt.en, fil: recText(inp.fmin).fil + ' ' + airTxt.fil }), inp.fmin <= REC.hiC ? 'alert' : '']);
...
CARDS.disease = function (root) {
  ...
  const form = el('form', { class: 'card-form', onsubmit: e => { e.preventDefault(); run(); } });
  const Tn = numInput('zT', { en: 'Air temperature this evening (°C)', fil: 'Temperatura ngayong gabi (°C)' }, prev.T, 0.1, { signed: true });
  ...
  const a1 = numInput('zA1', { en: 'Yesterday: lowest temperature (°C)', fil: 'Kahapon: pinakamababang temperatura (°C)' }, prev.a1, 0.1, { optional: true, signed: true }), a2 = numInput('zA2', { en: 'Yesterday: hours at or above 90% humidity', fil: 'Kahapon: oras na 90% pataas ang halumigmig' }, prev.a2, 0.5, { optional: true });
  const b1 = numInput('zB1', { en: 'Day before: lowest temperature (°C)', fil: 'Noong isang araw: pinakamababang temperatura (°C)' }, prev.b1, 0.1, { optional: true, signed: true }), b2 = numInput('zB2', { en: 'Day before: hours at or above 90% humidity', fil: 'Noong isang araw: oras na 90% pataas ang halumigmig' }, prev.b2, 0.5, { optional: true });
  ...
  function run() {
    ...
    if (inp.lwLo != null || inp.lwHi != null) {
      ...
      lwPanel = result({ level: 'info', verdict: t(T.verdicts[lwr.code], { lo: fmt(inp.lwLo, 1), hi: fmt(inp.lwHi, 1), blo: A.BLAST_WET.germLoH, bhi: A.BLAST_WET.germHiH,
          ...
          scopeF: lwr.straddlesBlast ? ' para sa mas basâng bahagi ng inyong lugar' : '', rlo: fmt(A.DEW_RICE_LB.nightLoH, 1), rhi: fmt(A.DEW_RICE_LB.nightHiH, 1) }),
        lines: lwr.straddlesBlast ? [[bi({ en: 'Your area is split', fil: 'Hati ang inyong lugar' }),
          bi({ en: 'The drier parts stay under ' + A.BLAST_WET.germLoH + ' h, but the wetter parts reach ' + fmt(inp.lwHi, 1) + ' h. Which one your own field is, only you can say.',
            fil: 'Ang mas tuyong bahagi ay wala pang ' + A.BLAST_WET.germLoH + ' oras, ngunit ang mas basâng bahagi ay umaabot sa ' + fmt(inp.lwHi, 1) + ' oras. Kung alin doon ang bukid ninyo, kayo lamang ang makapagsasabi.' })]] : null,
    ...
    if (inp.T != null && inp.RH != null) {
      ...
      show(out, result({ card: 'disease', level: (d.code === 'dew_less_likely') ? 'go' : 'caution', verdict: t(T.verdicts[d.code], { td: fmt(d.dewPoint, 1), dep: fmt(d.depression, 1) }), lines: (function () {
          ...
          if (d.code !== 'dew_less_likely') {
            ...
            ls.push([bi({ en: 'Whole wet period measured there', fil: 'Buong haba ng pagkabasâ roon' }),
          ...
          if (inp.zmin != null) {
            ...
            ls.push([bi({ en: 'PAGASA forecast minimum tonight', fil: 'Forecast ng PAGASA na pinakamababang temperatura ngayong gabi' }), bi(wf.reachesDewPoint
...
CARDS.timing = function (root) {
  ...
  const tx = numInput('tTx', { en: 'Typical afternoon high since planting (°C)', fil: 'Karaniwang pinakamainit mula nang itanim (°C)' }, prev.tx, 0.1, { optional: true, signed: true }), tn = numInput('tTn', { en: 'Typical morning low since planting (°C)', fil: 'Karaniwang pinakamalamig mula nang itanim (°C)' }, prev.tn, 0.1, { optional: true });
...
const READING = {
  water:   { en: 'Use the day\'s highest afternoon temperature and lowest morning temperature, not one reading taken now. A thermometer left in the shade, read in mid-afternoon and again at sunrise, gives both.', fil: 'Gamitin ang pinakamataas na temperatura sa hapon at pinakamababa sa umaga, hindi ang isang pagbása ngayon. Ang termometro sa lilim, basáhin sa kalagitnaan ng hapon at muli sa pagsikat ng araw.' },
  rice:    { en: 'Read the AWD tube (pani tube) in the morning, before you add any water that day.', fil: 'Basáhin ang AWD tube (pani tube) sa umaga, bago magdagdag ng tubig sa araw na iyon.' },
  ...
  spray:   { en: 'Take these readings where and when you intend to spray. Conditions change within the hour, so a reading from earlier in the morning does not describe the afternoon.', fil: 'Basáhin ito sa lugar at oras na balak mong mag-spray. Nagbabago ang kondisyon kada oras, kaya hindi naglalarawan ng hapon ang pagbása kaninang umaga.' },
  dry:     { en: 'Read the air beside the drying area in the early afternoon, when it is hottest and driest. Keep the instruments out of direct sun: in the sun a thermometer reads its own heated bulb, not the air, and the answer comes out too hopeful. A morning reading understates what the day can dry.', fil: 'Basáhin ang hangin sa tabi ng patuyuan sa maagang hapon, kung kailan pinakamainit at pinakatuyo. Huwag ilagay sa tuwirang araw ang mga kagamitan: sa araw, ang nababása ng termometro ay ang sarili nitong init, hindi ang hangin, kaya lumalabas na masyadong maganda ang sagot. Maliit ang ipinapakita ng pagbása sa umaga.' },
  stress:  { en: 'Use each day\'s actual afternoon high and morning low, not one reading taken now.', fil: 'Gamitin ang tunay na pinakamainit sa hapon at pinakamalamig sa umaga bawat araw, hindi ang isang pagbása ngayon.' },
  frost:   { en: 'Read as late as you can before sleeping. The air keeps cooling all night, so a reading taken at 6 or 7 p.m. can understate the risk. Take it outdoors, away from walls, at about head height.', fil: 'Magbása nang pinakahuli bago matulog. Patuloy na lumalamig ang hangin buong gabi, kaya maaaring maliitin ng pagbása sa alas-6 o alas-7 ng gabi ang panganib. Sa labas, malayo sa pader, mga kasintaas ng ulo.' },
...
const NEEDS = {
  ...
  rain:    { en: 'A rain gauge, read at the same hour each day and added up for the whole month. If you have none, PAGASA publishes monthly rainfall for its stations: use the nearest one, remembering that rainfall can differ a great deal between your field and a station some distance away. A phone weather app is a weaker fallback than the station record, because the rainfall it shows is an estimate for a wide area rather than what fell on your field.', fil: 'Sa panukat ng ulan (ulansukod), basáhin sa parehong oras araw-araw at ipunin sa buong buwan. Kung wala kayo nito, naglalathala ang PAGASA ng buwanang ulan sa mga istasyon nito: gamitin ang pinakamalapit, ngunit tandaan na malaki ang maaaring pagkakaiba ng ulan sa bukid ninyo at sa istasyong malayo. Mas mahina ang weather app sa smartphone kaysa sa talaan ng istasyon, dahil tantiya lamang iyon para sa malawak na lugar at hindi ang tunay na bumagsak sa bukid ninyo.' },
