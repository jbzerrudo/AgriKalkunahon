    ...
    const showDewPoint = function () {
      ...
      out.appendChild(result({ level: 'info',
        ...
        compare: dp.compare.map(function (c) { return c.name + ': ' + fmt(c.dewPoint, 2) + ' \u00b0C'; })
          .concat(['Spread across all four: ' + fmt(dp.compareSpreadC, 3) + ' \u00b0C. For comparison, a hygrometer reading three points of humidity off moves the dew point by roughly 0.6 to 0.9 \u00b0C, and a thermometer off by half a degree moves it by about half a degree. The method is not where the error is.']),
        ...
        limits: ['This describes the air where and when you read it. The dew point changes through the night, and a hollow does not hold the same air as the ridge above it, so one reading is a snapshot.',
                 ...
                 'The instrument sets the accuracy here, not the arithmetic. A hygrometer reading three points of humidity off moves the dew point by roughly 0.6 to 0.9 \u00b0C, and a thermometer off by half a degree moves it by about half a degree, both far larger than any difference between the published methods.',
