...
function locationWarnings() {
  ...
  const tz = tzHours(), tzs = 'UTC' + (tz >= 0 ? '+' : '') + tz;
  if (tz !== 8 && inPH()) out.push({ en: 'Your field is in the Philippines but this device clock is set to ' + tzs + '. Every hour the app shows you, sunrise, sunset, the spray inversion window and the frost reading window, is in this device\'s time, so it will not match the clocks around your field.', fil: 'Nasa Pilipinas ang bukid ninyo ngunit nakatakda sa ' + tzs + ' ang orasan ng device na ito. Lahat ng oras na ipinapakita ng app, ang pagsikat at paglubog ng araw, ang oras ng inversion sa pag-spray, at ang oras ng pagbasa para sa andap, ay nasa oras ng device na ito, kaya hindi ito tutugma sa mga orasan sa paligid ng bukid ninyo.' });
  return out;
}
...
CARDS.spray = function (root) {
  ...
  function run() {
    ...
    const L = store.loc, J = todayJ(new Date()), st = A.sunTimes(L.lat, L.lon, J, tzHours());
...
CARDS.frost = function (root) {
  ...
  function run() {
    ...
    const st = A.sunTimes(L.lat, L.lon, todayJ(new Date(now0.getTime() + 86400000)), tzHours());  // tomorrow's sunrise: the card is about tonight
    const stToday = A.sunTimes(L.lat, L.lon, todayJ(now0), tzHours());                            // tonight's sunset
    const hhmm = h => { const x = ((h % 24) + 24) % 24; return String(Math.floor(x)).padStart(2, '0') + ':' + String(Math.round((x % 1) * 60)).padStart(2, '0'); };
...
CARDS.disease = function (root) {
  ...
  function run() {
    ...
    const stT = A.sunTimes(L0.lat, L0.lon, todayJ(now0), tzHours()), stN = A.sunTimes(L0.lat, L0.lon, todayJ(new Date(now0.getTime() + 86400000)), tzHours());
