...
store.loc = store.loc || { lat: 14.6, lon: 121.0, elev: 20, site: 'coastal' };
...
const T = {
  ...
  ui: {
    ...
    assumptions: { en: 'Assumptions', fil: 'Mga palagay' }, limits: { en: 'Not valid for', fil: 'Hindi para sa' }, back: { en: 'Home', fil: 'Simula' },
    location: { en: 'Location', fil: 'Lokasyon' }, gps: { en: 'Use phone GPS', fil: 'Gamitin ang GPS ng telepono' },
...
function locationBlock(onchange) {
  ...
  const gps = el('button', { type: 'button', class: 'btn small', onclick: () => {
    ...
    navigator.geolocation.getCurrentPosition(p => { lat.input.value = p.coords.latitude.toFixed(3); lon.input.value = p.coords.longitude.toFixed(3); if (isFinite(p.coords.altitude)) elev.input.value = Math.round(p.coords.altitude); sync(); }, () => {}, { enableHighAccuracy: true, timeout: 8000 });
  } }, bi(T.ui.gps));
  function sync() { L.lat = +lat.input.value; L.lon = +lon.input.value; L.elev = +elev.input.value; L.site = site.input.value; save(); if (onchange) onchange(); }
  [lat, lon, elev, site].forEach(x => x.input.addEventListener('change', sync));
  wrap.append(gps, lat.row, lon.row, elev.row, site.row);
  return wrap;
}
...
CARDS.water = function (root) {
  ...
  const rhmax = numInput('rhmax', T.ui.rhmax, prev.rhmax, 1), rhmin = numInput('rhmin', T.ui.rhmin, prev.rhmin, 1);
  const wind = selectInput('wind', T.ui.wind, Object.keys(T.windClass).map(k => [k, T.windClass[k]]), prev.wind || 'unknown');
  const sun = numInput('sun', T.ui.sun, prev.sun, 0.5);
  const days = numInput('days', T.ui.daysSince, prev.days == null ? 5 : prev.days, 1);
  const rainRows = [0, 1, 2].map(i => { const mm = numInput('rmm' + i, { en: 'Rain ' + (i + 1) + ' (mm)', fil: 'Ulan ' + (i + 1) + ' (mm)' }, prev['rmm' + i], 0.5), ago = numInput('rago' + i, { en: 'days ago', fil: 'araw na nakalipas' }, prev['rago' + i], 1); return { mm, ago }; });
  const method = selectInput('method', T.ui.method, Object.keys(T.methods).map(k => [k, T.methods[k]]), prev.method || 'surface');
  const area = numInput('area', T.ui.area, prev.area == null ? 1 : prev.area, 0.01), pump = numInput('pump', T.ui.pump, prev.pump, 0.5);
...
CARDS.spray = function (root) {
  ...
  const wkmh = numInput('sWk', { en: 'or wind in km/h at 2 m', fil: 'o hangin sa km/h sa 2 m' }, prev.wkmh, 1);
  const label = numInput('sLbl', { en: 'Wind limit printed on the label, if any (km/h)', fil: 'Limitasyon ng hangin sa label, kung meron (km/h)' }, prev.label, 1);
  ...
  function run() {
    ...
    show(out, result({ level, verdict: t(T.verdicts[r.code]), lines, why, assumptions: ['Sunrise and sunset from FAO-56 Eq. 25 and 31 to 33 for Philippine time (UTC+8).', 'Wet bulb from FAO-56 Eq. 15 and 16 (ventilated psychrometer constant) at your elevation.'],
...
const NEEDS = {
  water:   { en: 'A thermometer in the shade, ideally one that keeps the day\'s highest and lowest. A rain gauge. Your location, which the phone can fill in. With no thermometer, a phone weather app\'s high and low for the nearest town will serve as a rough stand-in, but it is a forecast for somewhere else: expect error, and more of it if that town sits at a different elevation.', fil: 'Termometro sa lilim, mas mabuti kung may tanda ng pinakamataas at pinakamababa sa araw. Panukat ng ulan. Ang lokasyon ninyo, na kayang kunin ng telepono. Kung walang termometro, puwedeng gamiting pahapyaw na panghalili ang mataas at mababang temperatura ng pinakamalapit na bayan sa weather app ng telepono, ngunit hula iyon para sa ibang lugar: asahan ang pagkakamali, lalo na kung magkaiba ang taas ng lugar.' },
  ...
  rain:    { en: 'A rain gauge, read at the same hour each day and added up for the whole month. If you have none, PAGASA publishes monthly rainfall for its stations: use the nearest one, remembering that rainfall can differ a great deal between your field and a station some distance away. A phone weather app is a weaker fallback than the station record, because the rainfall it shows is an estimate for a wide area rather than what fell on your field.', fil: 'Panukat ng ulan, basahin sa parehong oras araw-araw at ipunin sa buong buwan. Kung wala kayo nito, naglalathala ang PAGASA ng buwanang ulan sa mga istasyon nito: gamitin ang pinakamalapit, ngunit tandaan na malaki ang maaaring pagkakaiba ng ulan sa bukid ninyo at sa istasyong malayo. Mas mahina ang weather app sa telepono kaysa sa talaan ng istasyon, dahil tantiya lamang iyon para sa malawak na lugar at hindi ang tunay na bumagsak sa bukid ninyo.' },
  spray:   { en: 'A thermometer and a hygrometer, taken at the spot you will spray. A watch, and some way to judge the wind. Do not take the wind from a phone weather app: drift is decided by the wind in your field at boom height, not at a station kilometres away and ten metres up, and the label holds you to the conditions where you spray.', fil: 'Termometro at hygrometer, sa mismong lugar na sisprayan. Relo, at paraan ng pagtantiya ng hangin. Huwag kunin sa weather app ng telepono ang hangin: ang pag-anod ng spray ay nakasalalay sa hangin sa mismong bukid ninyo sa taas ng boom, hindi sa istasyong ilang kilometro ang layo at sampung metro ang taas, at ang label ang panagot ninyo sa kondisyon kung saan kayo nagsi-spray.' },
  dry:     { en: 'A thermometer and a hygrometer, shaded from direct sun but standing in the air beside the drying area. A weighing scale. A moisture meter if you have one; otherwise your usual way of judging the grain. A phone weather app\'s humidity is measured somewhere else entirely and will not match your drying floor; use it only if you have nothing at all.', fil: 'Termometro at hygrometer, may silong mula sa tuwirang araw ngunit nasa hangin sa tabi ng patuyuan. Timbangan. Moisture meter kung meron; kung wala, ang nakasanayan ninyong paraan ng pagtingin sa butil. Ang humidity na ipinapakita ng weather app sa telepono ay sinukat sa ibang lugar at hindi tutugma sa bilaran ninyo; gamitin lamang kung talagang wala kayong ibang paraan.' },
  stress:  { en: 'A thermometer in the shade that keeps the day\'s highest and lowest. With none, a phone weather app\'s high and low for the nearest town will serve as a rough stand-in, but it is a forecast for somewhere else: expect error, and more of it if that town sits at a different elevation.', fil: 'Termometro sa lilim na may tanda ng pinakamataas at pinakamababa sa araw. Kung wala nito, puwedeng gamiting pahapyaw na panghalili ang mataas at mababang temperatura ng pinakamalapit na bayan sa weather app ng telepono, ngunit hula iyon para sa ibang lugar: asahan ang pagkakamali, lalo na kung magkaiba ang taas ng lugar.' },
  frost:   { en: 'A thermometer and a hygrometer, read outdoors away from walls, at about head height. This is the one card where a phone weather app will not serve. Frost forms by cold air draining into your own hollow, so two fields in the same barangay differ, and a town reading cannot see that. Take this one yourself.', fil: 'Termometro at hygrometer, basahin sa labas, malayo sa pader, mga kasintaas ng ulo. Ito ang card na hindi talaga kayang palitan ng weather app sa telepono. Nabubuo ang hamog na yelo kapag umaagos ang malamig na hangin pababa sa sariling lubak ninyo, kaya magkaiba ang dalawang bukid sa iisang barangay, at hindi iyon nakikita ng basa sa bayan. Kayo mismo ang dapat sumukat nito.' },
  disease: { en: 'A thermometer and a hygrometer, read outdoors near the crop in the evening. As with frost, a phone weather app is a poor substitute here, because dew depends on how your own field cools rather than on the town average.', fil: 'Termometro at hygrometer, basahin sa labas malapit sa pananim kinagabihan. Tulad sa hamog na yelo, mahina ang weather app sa telepono dito, dahil nakadepende ang hamog sa paglamig ng sarili ninyong bukid at hindi sa karaniwan ng bayan.' },
  timing:  { en: 'Your planting or sowing dates, and a thermometer for a typical high and low. A phone weather app is acceptable here, because this card wants a typical value for the season rather than a particular day.', fil: 'Ang petsa ng pagtatanim o pagpupunla, at termometro para sa karaniwang mataas at mababa. Puwede ang weather app sa telepono dito, dahil karaniwang halaga para sa panahon ang hinahanap ng card na ito at hindi ang isang tiyak na araw.' }
};
