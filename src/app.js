...
function locationBlock(onchange) {
  ...
  const gps = el('button', { type: 'button', class: 'btn small', onclick: () => {
    ...
    navigator.geolocation.getCurrentPosition(
      p => { lat.input.value = p.coords.latitude.toFixed(3); lon.input.value = p.coords.longitude.toFixed(3); if (isFinite(p.coords.altitude)) elev.input.value = Math.round(p.coords.altitude); sync(); },
  ...
  wrap.append(gps, note, lat.row, lon.row, elev.row, site.row, warn);
  redraw();
  return wrap;
}
...
const phTime = () => -new Date().getTimezoneOffset() / 60 === 8;
function locationWarnings() {
  ...
  if (isFinite(L.lat) && isFinite(L.lon) && (L.lat < 4 || L.lat > 22 || L.lon < 116 || L.lon > 127)) out.push({ en: 'This location is outside the Philippines. Every threshold in this app comes from Philippine or FAO sources chosen for Philippine conditions, and the frost card is built on Benguet records. Treat the answers as illustrations, not advice for this place.', fil: 'Nasa labas ng Pilipinas ang lokasyong ito. Lahat ng hangganan sa app na ito ay mula sa Pilipinas o FAO na piniling angkop sa kondisyon ng Pilipinas, at ang card ng andap ay nakabatay sa talaan ng Benguet. Ituring na halimbawa lamang ang mga sagot, hindi payo para sa lugar na ito.' });
  const tz = -new Date().getTimezoneOffset() / 60;
  if (tz !== 8) out.push({ en: 'Your device clock is set to UTC' + (tz >= 0 ? '+' : '') + tz + ', not Philippine time (UTC+8). Sunrise, sunset, the spray inversion window and the frost reading window all assume UTC+8, so every hour this app shows you will be out by ' + Math.abs(8 - tz) + ' hours.', fil: 'Nakatakda ang orasan ng device ninyo sa UTC' + (tz >= 0 ? '+' : '') + tz + ', hindi sa oras ng Pilipinas (UTC+8). Ang pagsikat at paglubog ng araw, ang oras ng inversion sa pag-spray, at ang oras ng pagbasa para sa andap ay lahat nakabatay sa UTC+8, kaya mali nang ' + Math.abs(8 - tz) + ' oras ang bawat oras na ipinapakita ng app.' });
  return out;
}
...
CARDS.frost = function (root) {
  ...
  function run() {
    ...
    if (phTime() && !A.frostReadingUsable(nowH, stToday.sunset, st.sunrise)) {
...
CARDS.disease = function (root) {
  ...
  function run() {
    ...
    if (phTime() && inp.T != null && inp.RH != null && !A.frostReadingUsable(nowH0, stT.sunset, stN.sunrise)) {
      show(out, result({ level: 'info',
...
const NEEDS = {
  water:   { en: 'A thermometer in the shade, ideally one that keeps the day\'s highest and lowest. A rain gauge. Your location, which the phone can fill in. With no thermometer, a phone weather app\'s high and low for the nearest town will serve as a rough stand-in, but it is a forecast for somewhere else: expect error, and more of it if that town sits at a different elevation.', fil: 'Termometro sa lilim, mas mabuti kung may tanda ng pinakamataas at pinakamababa sa araw. Panukat ng ulan. Ang lokasyon ninyo, na kayang kunin ng smartphone. Kung walang termometro, puwedeng gamiting pahapyaw na panghalili ang mataas at mababang temperatura ng pinakamalapit na bayan sa weather app ng smartphone, ngunit hula iyon para sa ibang lugar: asahan ang pagkakamali, lalo na kung magkaiba ang taas ng lugar.' },
  ...
  stress:  { en: 'A thermometer in the shade that keeps the day\'s highest and lowest. With none, a phone weather app\'s high and low for the nearest town will serve as a rough stand-in, but it is a forecast for somewhere else: expect error, and more of it if that town sits at a different elevation.', fil: 'Termometro sa lilim na may tanda ng pinakamataas at pinakamababa sa araw. Kung wala nito, puwedeng gamiting pahapyaw na panghalili ang mataas at mababang temperatura ng pinakamalapit na bayan sa weather app ng smartphone, ngunit hula iyon para sa ibang lugar: asahan ang pagkakamali, lalo na kung magkaiba ang taas ng lugar.' },
